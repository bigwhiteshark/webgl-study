...
export default class Ex4_3 {
  ...
  updateTransforms() {
    ...
    if (this.coords === this.COORDS_WORLD) {
      ...
      mat4.rotateX(mvMatrix, mvMatrix, this.rotation[0] * (Math.PI / 180));
      mat4.rotateY(mvMatrix, mvMatrix, this.rotation[1] * (Math.PI / 180));
      mat4.rotateZ(mvMatrix, mvMatrix, this.rotation[2] * (Math.PI / 180));
    } else {
      mat4.identity(cMatrix);
      mat4.rotateX(cMatrix, cMatrix, this.rotation[0] * (Math.PI / 180));
      mat4.rotateY(cMatrix, cMatrix, this.rotation[1] * (Math.PI / 180));
      mat4.rotateZ(cMatrix, cMatrix, this.rotation[2] * (Math.PI / 180));
  ...
  change() {
    ...
    const f1 = gui.addFolder('Translate');
    f1.add(controls, 'x', -50, 50).listen().onChange((value) => {
      this.position[0] = parseFloat(value);
      this.draw();
    });
    f1.add(controls, 'y', -50, 50).listen().onChange((value) => {
      this.rotation[1] = parseFloat(value);
      this.draw();
    });
    f1.add(controls, 'z', -50, 50).listen().onChange((value) => {
      this.rotation[2] = parseFloat(value);
      this.draw();
    });
    const f2 = gui.addFolder('Rotation');
    f2.add(controls, 'degreesX', -50, 50).listen().onChange((value) => {
      this.rotation[0] = +parseFloat(value);
      this.draw();
    });
    f2.add(controls, 'degreesY', -50, 50).listen().onChange((value) => {
      this.rotation[1] = +parseFloat(value);
      this.draw();
    });
    f2.add(controls, 'degreesZ', -50, 50).listen().onChange((value) => {
      this.rotation[2] += parseFloat(value);
      this.draw();
    });
    ...
    typeControl.onChange((value) => {
      ...
      this.rotation = [0, 0, 0];
      controls.x = this.rotation[0];
      controls.y = this.rotation[1];
      controls.z = this.rotation[2];
  ...
  load() {
    const vsCode = require('./glsl/Ex4_2.vs');
    const fsCode = require('./glsl/Ex4_2.fs');
