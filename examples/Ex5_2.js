...
export default class Ex5_2 {
  ...
  load() {
    ...
    const temp_ball = require('./models/geometry/ball.json');
    for (let i = 0; i < NUM_BALLS; i += 1) {
      this.balls.push(new BouncingBall());
      const ball = { ...temp_ball };
      ball.alias = 'ball';
      ball.index = i;
    ...
    try {
      ...
      for (let i = 0, l = scene.objects.length; i < l; i++) {
        ...
        if (object.alias === "ball") {
          ...
          mat4.translate(ballTransform, ballTransform, this.balls[index].position);
          object.diffuse = this.balls[index].color;
        }
  ...
  animate() {
    ...
    for (let i = 0; i < this.balls.length; i++) {
      this.balls[i].update(this.sceneTime);
    }
    this.sceneTime += 33 / 1000;
    this.draw();
  }
