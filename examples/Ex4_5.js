...
export default class Ex4_5 {
  constructor() {
    ...
    this.PROJ_ORTHO = 0;
    this.PROJ_PERSPECTIVE = 1;
    this.projectionMode = this.PROJ_PERSPECTIVE;
  ...
  updateTransforms() {
    ...
    if (this.projectionMode === this.PROJ_PERSPECTIVE) {
      mat4.perspective(pMatrix, 30 * (Math.PI / 180), this.viewWidth / this.viewHeight, 10, 5000);
    } else {
      mat4.ortho(pMatrix, -this.viewWidth, this.viewWidth, -this.viewHeight, this.viewHeight, -5000, 5000)
    }
  }
  ...
  change() {
    ...
    const pControl = gui.add(controls, 'PerspectiveType', { perspective: 1, orthographic: 0 });
    ...
    pControl.onChange((value) => {
      this.projectionMode = parseInt(value, 0);
      this.draw();
    });
  ...
  load() {
    const vsCode = require('./glsl/Ex4_5.vs');
    const fsCode = require('./glsl/Ex4_5.fs');
    ...
    for (let i = 1; i < 178; i++) {
      const object = require(`./models/nissan_gtr/pr${i}.json`);
